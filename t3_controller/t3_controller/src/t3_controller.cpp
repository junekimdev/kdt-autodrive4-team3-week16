...
const std::string SUB_TOPIC_LIDAR = "scan";
...
const cv::Mat PROJECT_MATRIX = cv::Mat_<float>(
    { 3, 4 }, { -3.44775116e+02, 3.27100769e+02, -1.43375635e+01, 9.36617661e+00, -2.52258957e+02, -2.30760689e+01,
                -3.51428497e+02, 3.69152336e+01, -9.97794509e-01, -6.26784712e-02, -2.18525622e-02, 4.97312471e-02 });
const cv::Mat CAMERA_MATRIX =
    cv::Mat_<float>({ 3, 3 }, { 348.14820298, 0., 323.82585721, 0., 345.88660809, 260.82857638, 0., 0., 1. });

const cv::Mat DISTORTION_COEFFICIENTES_ERROR =
    cv::Mat_<float>({ 1, 5 }, { -0.3460703, 0.14435681, 0.00177496, 0.00118325, -0.0321834 });
...
public:
  ...
  void callbackLidar(const sensor_msgs::LaserScan::ConstPtr& msg);
...
void Controller::control()
{
  ...
  if (sensor_state.object.boundingBoxes.size() > 0)
  {
    for (auto objectBox : sensor_state.object.boundingBoxes)
    {
  ...
  for (auto ptr : sensor_state.lidar.ranges)
  {
